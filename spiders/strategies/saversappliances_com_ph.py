...
class SaversappliancesComPhStrategy(SpiderStrategy):
    ...
    def execute(self):
        ...
        while True:
            ...
            if raw_data:
                ...
                product_tags = soup.select('ul.products > li')
                next_page = soup.select_one('nav.woocommerce-pagination a.next')
    ...
    def _get_title(self, product_tag: BeautifulSoup):
        ...
        try:
            title_tag = product_tag.select_one('.product-title > h3')
            if title_tag:
    ...
    def _url(self, product_tag: BeautifulSoup):
        ...
        try:
            url_tag = product_tag.select_one('a.product-title')
    ...
    def _get_price(self, product_tag: BeautifulSoup):
        value = 0.0
        try:
            price_tag = product_tag.select_one('ins .woocommerce-Price-amount.amount') 
            if price_tag:
    ...
    def _image_url(self, product_tag: BeautifulSoup):
        ...
        try:
            image_tag = product_tag.select_one('.product-image img')
