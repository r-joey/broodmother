...
class MangkosmeComStrategy(SpiderStrategy):
    ...
    def _download(self, headers=None, cookies=None, timeout=10, data=None):
        ...
        try:  
            ...
            headers = {  
                ...
                'if-none-match': '"cacheable:b8049de69110c43840c333a8e9c38c1f"',
    ...
    def execute(self):
        ...
        while True:
            ...
            if raw_data:
                ...
                product_tags = soup.select('div#collection-products > div.product')
                next_page = soup.select_one('ul.pagination a[title="Next"]')
                ...
                if next_page:
                    self.url = f'https://mangkosme.com{next_page['href']}'
                    time.sleep(10)
                else:
                    break
            else:
    ...
    def _get_title(self, product_tag: BeautifulSoup):
        ...
        try:
            title_tag = product_tag.select_one('div.product-title > h4 > a')
            if title_tag:
    ...
    def _url(self, product_tag: BeautifulSoup):
        ...
        try:
            url_tag = product_tag.select_one('div.product-title > h4 > a')
            if url_tag and url_tag.has_attr('href'):
                value = f"https://mangkosme.com{url_tag['href']}"
        except Exception as e:
            value = 'FAILED'
            self.logger.exception(e)
    ...
    def _get_price(self, product_tag: BeautifulSoup):
        ...
        try:
            price_tag = product_tag.select_one('.product-price > h6')
    ...
    def _image_url(self, product_tag: BeautifulSoup):
        ...
        try:
            image_tag = product_tag.select_one('.product-thumb img')
            if image_tag and image_tag.has_attr('data-src'):
                value = image_tag['data-src']
        except Exception as e:
            value = 'FAILED'
            self.logger.exception(e)
